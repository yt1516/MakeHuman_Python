...
class ExampleTaskView(gui3d.TaskView):
    ...
    def __init__(self, category, appFacs):
        ...
        self.au43_lab=box_weight.addWidget(gui.TextView('AU43 Weight'))
        self.au43_val=box_weight.addWidget(gui.TextEdit(text='0'))
                ...
                for i7 in range(3):
                    for i9 in range(3):
                        for i10 in range(3):
                            for i27 in range(3):
                                for i43 in range(3):
                                    self.survey_img_gen(weight[i4],weight[i7],weight[i9],weight[i10],weight[i27],weight[i43])
                                    time.sleep(0.5)
                                    self.renderSurveyImg(i4,i7,i9,i10,i27,i43)
...
##########################################################################
    ...
    def survey_img_gen(self,i4,i7,i9,i10,i27,i43):
        for key_code in self.slidersValues.keys():
            if key_code=='4':
                ...
                self.sliders[key_code].onChanging(i27)
                self.labelSlider[key_code].setTextFormat( 'Intensity: %.2f%%', self.sliders[key_code].getValue()*100)
            elif key_code=='43':
                self.sliders[key_code].onChanging(i43)
                self.labelSlider[key_code].setTextFormat( 'Intensity: %.2f%%', self.sliders[key_code].getValue()*100)
            else:
    ...
    def renderSurveyImg(self, i4,i7,i9,i10,i27,i43,dir_images = None, pic_file = None, pic_file_reverse = None):
       self.facs_human.applyAllTargets()
       ...
       if pic_file is not None:
          ...
          if pic_file_reverse is not None:
           ...
           grabName = str(i4)+str(i7)+str(i9)+str(i10)+str(i27)+str(i43)+'.png'
    ...
    def generate_face_set(self,val):
        ...
        au43=float(self.au43_val.getText())
        beta=[au4,au7,au9,au10,au43]
        ...
        for key_code in self.slidersValues.keys():
            if key_code=='4':
                ...
                self.sliders[key_code].onChanging(multiplier*weight[4])
                self.labelSlider[key_code].setTextFormat( 'Intensity: %.2f%%', self.sliders[key_code].getValue()*0)
            else:
                self.sliders[key_code].onChanging(0)
            self.sliders[key_code].update()
        self.facs_human.applyAllTargets()
    ...
    def renderPainSet(self, val):
        ...
        if self.renderingWidth == '' or self.renderingHeight == '' :
            ...
            img_width, img_height  = 500,500#int(self.renderingWidth), int(self.renderingHeight)
            glmodule.draw(False)
            img = glmodule.renderToBuffer(img_width, img_height)
            alphaImg = glmodule.renderAlphaMask(img_width, img_height)
            img = imgop.addAlpha(img, imgop.getChannel(alphaImg, 0))
